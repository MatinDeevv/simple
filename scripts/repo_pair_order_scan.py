...
``config/instruments.json`` (loaded through ``pipeline/contracts.py``) is the
...
EXCLUDED_DIR_NAMES = {"tests", "scripts", "__pycache__", ".venv-quantum", ".git", "node_modules"}
...
def _production_python_files(root: Path) -> list[Path]:
    ...
    if pipeline_dir.is_dir():
        files.extend(sorted(pipeline_dir.glob("*.py")))
    return files
