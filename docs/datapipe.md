# Data Pipeline â€” Ingestion Spec (OQ-11)

| | |
|---|---|
| **Version** | 1.0.0 (= `PIPELINE_VERSION` in `fxresearch/data/ingestion/ingest.py`) |
| **Status** | **OQ-11 RESOLVED** â€” idempotent sort/dedupe/validate ingestion implemented, run, and verified; canonical stream is `data/canonical/<PAIR>.parquet` + `data/canonical/manifest.json`. |
| **Owner** | sim-datapipe |
| **Date** | 2026-07-17 |
| **Upstream** | `data/raw/dukascopy/<PAIR>.csv` (raw; HC-3: never assumed sorted; not a valid input to any other agent) |
| **Downstream** | canonical stream only â€” schema Â§5 step 0 |
...
Run: `python pipeline\ingest.py` (all 10 pairs, tracked index order from `fxresearch/config/instruments.json` per schema Â§2). `--pairs P1 P2 ...` ingests a subset and merges entries into the existing manifest.

Per pair `P`:

1. **Hash source.** SHA256 of the raw file bytes `data/raw/dukascopy/P.csv` â†’ `source_sha256`.
...
| V4 | OHLC sanity: `low â‰¤ open`, `low â‰¤ close`, `open â‰¤ high`, `close â‰¤ high`, `low â‰¤ high` | soft | count + log offending bars (first 5 sampled into manifest). **Rows are kept unmodified â€” no clipping, no repair.** |
| V5 | Timestamps minute-aligned (`epoch_us % 60e6 == 0`) | soft (informational) | count + log |
| V6 | `volume â‰¥ 0` | soft (informational) | count + log |
...
**Gaps are preserved, never filled, never interpolated.** A missing minute in raw stays a missing row in canonical. No resampling, no forward-fill, no synthetic bars, no weekend bridging. Downstream owns gap semantics: `Î”t`/`g_gap` bookkeeping is schema Â§4.2, and integration across gaps is **OQ-8 (sim-integrator)**. Any downstream code that needs a dense minute grid must build it itself, explicitly, on its own version stamp â€” silent interpolation inside the pipeline is treated as a defect.
...
data_sha256 = SHA256(
  ...
  â€– int64-LE bytes of UTC epoch SECONDS of every bar timestamp, ascending
  â€– float64-LE (IEEE-754) raw bytes of column open   (same row order)
  â€– float64-LE raw bytes of column high
  â€– float64-LE raw bytes of column low
  â€– float64-LE raw bytes of column close
  â€– float64-LE raw bytes of column volume
)
```
...
Top level: `pipeline_version`, `canonical_hash_spec` (the definition string), `generated_utc` (only non-deterministic field), `environment` (python/pandas/numpy/pyarrow versions â€” lineage, not identity), `raw_dir`, `instrument_index_order` (copied from tracked `fxresearch/config/instruments.json` and validated against it), `pairs`.

Per-pair entry (all fields deterministic given raw bytes + pipeline version):

| Field | Meaning |
|---|---|
| `index` | frozen instrument index (schema Â§2) |
| `source_file`, `source_sha256` | raw CSV path (repo-relative, posix) and its byte hash |
| `rows_in`, `rows_out`, `duplicates_dropped` | `rows_out = rows_in âˆ’ duplicates_dropped`, always |
...
| `data_sha256` | canonical identity (Â§4) |
...
1. `source_sha256`: hash `data/raw/dukascopy/<PAIR>.csv` bytes; must match manifest. If it does not, the raw file changed after ingestion (e.g. HC-3 append) â€” the canonical stream is stale and must be re-ingested; that is a detection, not a failure of this spec.
2. Re-run `python pipeline\ingest.py` (or an independent reimplementation of Â§1) and confirm the per-pair entries are identical to the committed manifest.
