...
| Default command | `python fxresearch/models/classical/estimate_coupling.py` |
...
This is a classical, time-varying dependence field. â€œCorrelationâ€ has no quantum-mechanics meaning.
...
The estimator never treats the three cross arithmetic relations as discovered dependence. Define `z=Tg`, `b=Ta`, where channels 0â€“6 remain the raw returns/accelerations and:
...
and identically for `b`. Thus channels 7â€“9 are cross-quote residual movements. `T` is invertible, so the fit is mapped back to the schemaâ€™s raw-instrument convention:
...
This construction retains independent major and non-triangle pair channels while representing EURGBP/EURJPY/GBPJPY only through deviations from their stipulated arithmetic relationships during estimation. It is not â€œzeroing the pairs.â€
...
Before 28,800 usable samples, a daily diagnostics row is recorded with `INSUFFICIENT_HISTORY`; no matrix is invented. A non-finite fit or condition number over `1e12` records `REJECTED_NUMERICAL_CONDITION`; no zero, identity, or stale substitute matrix is written for that time. A consumer must hold its previously accepted field or explicitly decouple/re-initialize under the integratorâ€™s policy, while preserving the rejection event.
