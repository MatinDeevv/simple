...
|---|---|
| Owner | sim-coupling |
| Input | `data_canonical/<PAIR>.parquet` only; raw CSV is never read |
| Instrument order | `[EURUSD, USDJPY, GBPUSD, AUDUSD, USDCAD, USDCNH, USDCHF, EURGBP, EURJPY, GBPJPY]` |
| Default command | `python pipeline/estimate_coupling.py` |
| Output | `data_derived/coupling_estimates.parquet`, `coupling_diagnostics.parquet`, `coupling_summary.json`, `coupling_validation.json` |
