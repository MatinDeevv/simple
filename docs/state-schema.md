...
| OQ-14 | **FROZEN v0.2 CONTRACT 2026-07-18** (D-026, D-028, D-029): identity-free two-regime residual-level arena with frozen entry bases/scales/horizon, currency-incidence and selected-factor diagnostic neutrality, graph-conditioned basket selection, a frozen-basket primary label, conditional climatology, and real-time-duration bootstrap blocks. All data end in burned 2024, so v0.2 is synthetic-test-only pending new post-2024 data. | Chief Systems Architect | blocks any empirical v0.2 score, predictive model promotion, and quantum-branch revisit until a new untouched holdout and matched ablations exist |
| OQ-15 | **IMPLEMENTED CONTRACT; BLOCKED DATA 2026-07-18** (D-027, D-029): legal/regulatory event ledger, citation-time DAG validation, scenario-weighted pair-exposure vectors, strict-after-known-time gap-safe FX event study, and a scaled pre-event-drift expected-return baseline; a timestamped primary-source corpus and entity/outcome linkage are absent | Chief Systems Architect | blocks legal-event evaluation, causal claims, and any portfolio use |
