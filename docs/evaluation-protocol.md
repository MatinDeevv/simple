...
- `engine/evaluation/evaluation_protocol.py` — purge/embargo/cluster helpers for any
  chronological train/OOS split.
- `engine/evaluation/entry_diagnostics.py` — post-hoc entry-policy and overlap
  annotation for arena emissions.

Neither module edits `engine/models/statistical/stat_arb.py` or `engine/models/events/legal_event.py`.
...
`engine/models/statistical/stat_arb.py` emits one research row per eligible minute. Whether
...
- It does not change `engine/models/statistical/stat_arb.py`'s frozen v0.1/v0.2 evaluation,
