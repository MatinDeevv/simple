...
- `fxresearch/evaluation/evaluation_protocol.py` â€” purge/embargo/cluster helpers for any
  chronological train/OOS split.
- `fxresearch/evaluation/entry_diagnostics.py` â€” post-hoc entry-policy and overlap
  annotation for arena emissions.

Neither module edits `fxresearch/models/statistical/stat_arb.py` or `fxresearch/models/events/legal_event.py`.
...
boundary into the OOS period â€” nor does it say anything about how many
...
evaluation â€” `is_training` is supplied by the caller exactly as already
computed by the frozen chronological split. It only *annotates*:

- `target_start_index`, `target_end_index` â€” the row's outcome window.
- `target_cluster_id` â€” connected-component id: rows whose outcome windows
...
- `purged_from_training` â€” True for a training row whose outcome window
  overlaps *any* evaluation-partition row's outcome window.
- `embargoed` â€” the same, but against evaluation windows widened by
...
alongside â€” never in place of â€” the frozen contract result. Recommended
...
`fxresearch/models/statistical/stat_arb.py` emits one research row per eligible minute. Whether
...
its horizon is open, is a real modeling choice â€” not something this branch
should decide silently inside the arena's per-minute loop. Four contracts:

| Policy | Rule |
|---|---|
| `independent_research_entries` (**frozen default**) | every eligible row counts, regardless of overlap â€” this is what the archived v0.1/v0.2 evaluation already does today, so naming it the default does not silently change any archived result's meaning |
...
(minute-over-minute change in the raw candidate basket â€” this is what the
...
*accepted* entries under the chosen policy â€” `NaN` everywhere else). Keeping
...
- It does not change `fxresearch/models/statistical/stat_arb.py`'s frozen v0.1/v0.2 evaluation,
  bootstrap, or promotion logic.
- It does not pick a "correct" entry policy â€” that remains a modeling
