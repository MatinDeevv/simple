...
This document describes what `scripts/verify_repository.py` and
...
python scripts/verify_repository.py --skip-slow   # cheap static checks only
```
...
|---|---|---|
| tracked configuration files exist | `config/instruments.json`, `config/legal-event-schema.json`, `config/frozen-archives.json`, both requirements files, and `ci.yml` are tracked by git, present, and (for JSON) parse | that their *content* is semantically correct beyond JSON validity |
| clean-checkout imports | `git archive HEAD` into a temp dir, then `python -c "import <module>"` for every tracked `pipeline/*.py`, subprocess per module | that the module *runs* correctly, only that it *imports* without needing `data_canonical/`, `data_derived/`, or any other generated/gitignored path |
...
| required module self-checks | `simulate_integrator.py`, `stat_arb.py`, `legal_event.py` `--self-check`, each on deterministic synthetic input | anything about real canonical-data behavior; self-checks never touch `data_canonical/` |
...
| state schemas valid | every `config/schemas/*.schema.json` is well-formed under `pipeline/schema_validate.py`'s subset | that any specific *artifact* on disk currently conforms — see [Schema validation](#schema-validation) below |
| shared instrument order | `pipeline/contracts.canonical_pair_order` loads and resolves to ten unique pairs, and no production module hardcodes a duplicate copy (`scripts/repo_pair_order_scan.py`) | — |
...
| no empty required source files | every `pipeline/*.py`, `tests/*.py`, `scripts/*.py` on disk has non-whitespace content | — |
| frozen archives unchanged | `config/frozen-archives.json` registry: for each entry present on disk, its sha256 (and recorded `version` field) matches the frozen value | if the file is absent (expected in a clean checkout / CI, since `data_derived/` is gitignored), this is a pass, not a failure — absence is not validity, but it is also not a violation of "don't overwrite" |
| burned-holdout guard enabled | invokes `stat_arb.py` with no flags; asserts nonzero exit, a refusal message mentioning "burned", and that `data_derived/` contents are byte-for-byte unchanged before/after | this check depends on `pipeline/stat_arb.py` internals (the `--allow-burned-holdout-research` flag) that are owned by other in-flight work; if that flag is ever renamed or removed, this check fails loudly by design |

## Schema validation

`pipeline/schema_validate.py` implements a small, dependency-free subset of
JSON Schema (`type`, `required`, `properties`, `items`, `enum`,
`additionalProperties`) — not a full JSON Schema library. `config/schemas/`
...
`scripts/verify_repository.py`'s "state schemas valid" check only validates
that the schema *documents* are well-formed. It does not validate live
`data_derived/*.json` artifacts against them — those files are gitignored, so
...
- **core-contracts** — byte-compiles `pipeline`, `tests`, `scripts`; runs
  `tests/test_contracts.py`; runs `scripts/verify_repository.py`.
...
- **repository-audit** — `scripts/verify_repository.py` plus this branch's own
...
None of the six core jobs require `data_canonical/`, `data_derived/`, or
