...
External launchers configured in plugin `.mcp.json`:

- Perplexity: requires `PERPLEXITY_API_KEY` only in process environment.
- Firecrawl: requires `FIRECRAWL_API_KEY` only in process environment.
