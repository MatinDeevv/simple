...
Contract (see docs/datapipe.md and docs/state-schema.md HC-3, section 5 step 0):
  * Raw CSVs in data/raw/dukascopy/ are NEVER assumed sorted (HC-3: downloader
  ...
  * This script: reads raw -> stable-sorts by timestamp -> drops exact
    ...
    writes data/canonical/<PAIR>.parquet + data/canonical/manifest.json.
...
Run:  python -m fxresearch.data.ingestion.ingest            (all 10 pairs)
      python -m fxresearch.data.ingestion.ingest --pairs EURUSD GBPUSD
"""
...
RAW_DIR = ROOT / "data" / "raw" / "dukascopy"
...
def ingest_pair(pair: str, index: int, backend: str) -> dict:
    ...
    entry = {
        "index": index,
        "source_file": f"data/raw/dukascopy/{pair}.csv",
        ...
        **soft,
        ...
        "output_file": f"data/canonical/{out_path.name}",
