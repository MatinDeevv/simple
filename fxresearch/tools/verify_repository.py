...
Usage:
    python -m fxresearch.tools.verify_repository
...
OPTIONAL_ENV_MODULES = {
    "research.quantum.quantum_aer_noise": "qiskit_aer",
    "fxresearch.visualization.render_quantum_figures": "matplotlib",
}

REQUIRED_SELF_CHECKS = (
    "fxresearch.models.classical.simulate_integrator",
    "fxresearch.models.statistical.stat_arb",
    "fxresearch.models.events.legal_event",
)
ARCHIVE_SELF_CHECKS = (
    "research.quantum.quantum_lindblad",
    "research.quantum.quantum_trajectories",
    "research.quantum.quantum_process_tomography",
    "research.quantum.quantum_mps",
    "research.quantum.quantum_kernel",
    "research.quantum.quantum_reservoir",
)
CLEAN_IMPORT_MODULES = (
    "fxresearch.core.contracts", "fxresearch.core.run_manifest", "fxresearch.core.schema_validate",
    "fxresearch.data.ingestion.ingest", "fxresearch.evaluation.entry_diagnostics",
    "fxresearch.evaluation.evaluation_protocol", "fxresearch.models.classical.estimate_coupling",
    "fxresearch.models.classical.estimate_dynamics", "fxresearch.models.classical.simulate_integrator",
    "fxresearch.models.events.legal_event", "fxresearch.models.statistical.stat_arb",
    "fxresearch.visualization.render_quantum_figures", "research.quantum.quantum_aer_noise",
    "research.quantum.quantum_kernel", "research.quantum.quantum_lindblad",
    "research.quantum.quantum_mps", "research.quantum.quantum_process_tomography",
    "research.quantum.quantum_reservoir", "research.quantum.quantum_trajectories",
)
...
def check_tracked_configuration_files_exist() -> CheckResult:
    required = [
        "fxresearch/config/instruments.json",
        "fxresearch/config/legal-event-schema.json",
        "fxresearch/config/frozen-archives.json",
...
def _export_clean_checkout(dest: Path) -> None:
    """Build a code-only working-tree copy without generated datasets/artifacts."""
    for directory in ("fxresearch", "research", "tests"):
        shutil.copytree(ROOT / directory, dest / directory, ignore=shutil.ignore_patterns("__pycache__"))
    for filename in ("pyproject.toml", "requirements-core.txt", "requirements-quantum.txt", ".gitignore"):
...
def check_quantum_aer_self_check() -> CheckResult:
    ...
    ok, detail = _run_self_check("research.quantum.quantum_aer_noise")
...
def check_state_schemas_valid() -> CheckResult:
    ...
    from fxresearch.core.schema_validate import validate_schema_document
    problems = []
    for path in schemas:
        try:
...
def check_frozen_archives_unchanged() -> CheckResult:
    registry_path = ROOT / "fxresearch" / "config" / "frozen-archives.json"
...
def check_burned_holdout_guard_enabled() -> CheckResult:
    ...
    before = sorted(p.name for p in data_derived.glob("*") if p.name != ".gitkeep") if data_derived.is_dir() else []
    proc = subprocess.run([sys.executable, "-m", "fxresearch.models.statistical.stat_arb"], cwd=ROOT,
                          capture_output=True, text=True, timeout=60)
    after = sorted(p.name for p in data_derived.glob("*") if p.name != ".gitkeep") if data_derived.is_dir() else []
    if after != before:
